...
summary_table <- data.frame(
  ...
  mutate(
    ...
    Mean = formatC(Mean, format = "e", digits = 2),
...
- Budget has a strong positive effect on revenue, confirming that higher budgets tend to generate higher box office returns. Though the estimate coefficient for Budget is very small, but budget itself is  on a very large scale (around e08). So when interpreted at a realistic scale (e.g., per million dollars), an increase of $1 million in budget is associated with an approximate 4.2% increase in revenue, which is a meaningful and practically significant effect.
...
performance_table <- data.frame(
  Metric = c("MAE", "RMSE", "R²"),
  Value = c(formatC(mae, format = "e", digits = 2),
            formatC(rmse, format = "e", digits = 2),
...
importance_display <- importance_df %>%
  mutate(
    ...
    "Increase in Node Purity"= formatC(IncNodePurity, format = "e", digits = 2),
  ) |> 
...
# Plot 1: %IncMSE
plot_mse <- ggplot(importance_plot, aes(x = reorder(Feature, `%IncMSE`), y = `%IncMSE`)) +
...
plot_purity <- ggplot(importance_plot, aes(x = reorder(Feature, IncNodePurity), y = IncNodePurity)) +
